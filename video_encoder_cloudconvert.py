...
LOG_FILE = 'video_encoding_cloudconvert.log'
CSV_LOG_FILE = 'video_encoding_cloudconvert_log.csv'
ENCODED_FILES_JSON = 'encoded_files_cloudconvert.json'
TEMP_DIR = '/tmp/cloudconvert_temp'

# Async configuration
MAX_CONCURRENT_JOBS = 3  # Aynı anda maximum kaç video encode edilsin

# Video encoding settings for file size reduction with CloudConvert
ENCODING_SETTINGS = {
    'output_format': 'mp4',
    'video_codec': 'h265',
    'audio_codec': 'aac',
...
class AsyncVideoEncoderCloudConvert:
    def __init__(self):
    ...
    def encode_video(self, input_file_path: str) -> Optional[str]:
        ...
        try:
            self.logger.info(f"Starting CloudConvert encoding of {input_file_path}")
            
            # Get original file size
            original_size = self.get_file_size(input_file_path)
            
            # Use encoding settings
            ...
            self.logger.info(
                f"CloudConvert encoding completed. Original: {original_size} bytes, "
    ...
    async def process_videos_async(self, video_files: List[str]) -> int:
        ...
        self.logger.info(f"Starting concurrent CloudConvert processing of {len(video_files)} videos with max {MAX_CONCURRENT_JOBS} concurrent jobs")
...
async def main():
    ...
    if len(sys.argv) < 2:
        print("Usage:")
        print("  python video_encoder_cloudconvert.py all                    # Process all folders")
        print("  python video_encoder_cloudconvert.py today                  # Process today's folder")
        print("  python video_encoder_cloudconvert.py YYYY-MM-DD             # Process specific date")
        print("  python video_encoder_cloudconvert.py YYYY-MM-DD YYYY-MM-DD  # Process date range")
        sys.exit(1)
    
    try:
        encoder = AsyncVideoEncoderCloudConvert()
        
        if sys.argv[1] == 'all':
            # Process all folders
            total = await encoder.scan_all_folders_async()
            print(f"Total files processed with CloudConvert: {total}")
            
        elif sys.argv[1] == 'today':
            # Process today's folder
            today = datetime.now()
            total = await encoder.scan_date_folder_async(today.year, today.month, today.day)
            print(f"Files processed today with CloudConvert: {total}")
            ...
            print(f"Files processed for {date_str} with CloudConvert: {total}")
            ...
            print(f"Files processed in range with CloudConvert: {total}")
